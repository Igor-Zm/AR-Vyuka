...
using UnityEngine.UI;
    ...
    public Vector2 CalculatePosition(Vector2 normalized) {
        if(canvas == null)
            canvas = GetComponentInParent<Canvas>();
        //Screen.width)
        //canvas.pixelRect.width 
        //float ar = canvas.pixelRect.height / canvas.pixelRect.width;
        float ar = 1f;
        return new Vector2(normalized.x * (canvas.pixelRect.width/canvas.transform.localScale.x), normalized.y * (canvas.pixelRect.height / canvas.transform.localScale.y));
    }
